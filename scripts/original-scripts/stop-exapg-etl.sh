#!/bin/bash
# ExaPG - ETL-Framework stoppen
# Dieses Skript stoppt die ExaPG-Umgebung mit ETL-Framework
...
# Funktion zum Stoppen der ExaPG-Umgebung mit ETL-Framework
stop_exapg_etl() {
    echo -e "${BLUE}Stoppe ExaPG mit ETL-Framework...${NC}"
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}ExaPG mit ETL-Framework wurde erfolgreich gestoppt.${NC}"
        ...
        if [[ "$delete_volumes" =~ ^[jJ]$ ]]; then
            ...
            docker volume rm $(docker volume ls -q | grep -E "exapg_exapg-data|exapg_kafka-data|exapg_airflow-data") 2>/dev/null || true
            echo -e "${GREEN}Docker-Volumes wurden gelöscht.${NC}"
        else
            echo -e "${GREEN}Docker-Volumes wurden beibehalten.${NC}"
        fi
    else
        echo -e "${RED}Fehler beim Stoppen von ExaPG mit ETL-Framework.${NC}"
        exit 1
    fi
}
...
# Hauptfunktion
main() {
    echo -e "${BLUE}ExaPG - ETL-Framework stoppen${NC}"
    echo -e "${BLUE}===========================${NC}"
    
    # Stoppe ExaPG mit ETL-Framework
    stop_exapg_etl
}
